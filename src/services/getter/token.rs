...
pub async fn get_token_total_supply_testnet(token_address:&str) -> String {
    ...
    let provider = init_base_testnet().await; 
    ...
    if !wallet.expect("REASONS").is_empty(){
        ...
        match name {
            Ok(name) => {
                ...
                return format!(
                    "The Base Testnet token name {:#?}, symbol: 
            ...
            Err(_e) => {
                ...
                return format!(
                    "The token address {:#?} doesn't exist on Base Testnet, check if it's an NFT or wallet.
    ...
    if !wallet.expect("REASONS").is_empty(){
        ...
        match name {
            ...
            Err(_e) => {
                ...
                return format!(
                    "The token address {:#?} doesn't exist on base, check if it's an NFT or wallet.
                    If it doesn't still exist, then its likely on another chain or doesn't exist",
                token_addr)
            }
        }
...
pub async fn get_token_details_testnet(token_address:&str) -> String {
    ...
    if !wallet.expect("REASONS").is_empty(){
        ...
        Err(_e) => {
            ...
            return format!(
                "The token address {:#?} doesn't exist as a fungible token on The Base Testnet, check if it's an NFT. If it doesn't still exist, then its likely on another chain or doesn't exist",
            token_addr)
        }
        }
    }else{
    ...
    if !wallet.expect("REASONS").is_empty() {
        ...
        match decimal {
            ...
            Err(_e) => {
                ...
                return format!(
                    "The token address {:#?} doesn't exist as a fungible token on Base, check if it's an NFT.
                    If it doesn't still exist, then its likely on another chain or doesn't exist",
                token_addr)
    ...
    if !wallet.expect("REASONS").is_empty(){
        ...
        match token_decimal{
            ...
            Err(_e) => {
                ...
                return format!(
                    "The token address {:#?} does not exist as a Fungible Token, Check if it's an NFT or a contract on Base, else, it will likely be on another blockchain",token_addr
                );
            }
        }
    }else{
